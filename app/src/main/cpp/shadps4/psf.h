...
//
// PSF (param.sfo) parser for the Android port. Direct port of
// shadPS4's src/core/file_format/psf.{h,cpp} with `Common::FS::IOFile`
// replaced by `std::ifstream` and the logging macros no-op'd, so the
// file compiles standalone inside an NDK module without pulling the
// rest of the `common/` tree.
...
public:
    ...
    // Open a param.sfo file from disk. Returns false on I/O error or invalid
    // magic.
    bool Open(const std::filesystem::path& filepath);

    // Open a param.sfo file from an in-memory buffer. Returns false on invalid
    // magic or truncated buffer.
    ...
    [[nodiscard]] const std::vector<Entry>& GetEntries() const { return entry_list; }
    [[nodiscard]] std::chrono::system_clock::time_point GetLastWrite() const { return last_write; }
...
private:
    ...
    [[nodiscard]] std::pair<std::vector<Entry>::const_iterator, size_t>
    FindEntry(std::string_view key) const;
};
